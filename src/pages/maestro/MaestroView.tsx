...
const MaestroView = () => {
  ...
  // Lógica de Inteligencia de UI (Basada en Género)
  const isFemale = user.gender === 'Femenino';
  
  // Paleta dinámica de Tailwind (Azul para hombres, Rosa para mujeres)
  const theme = {
    bgTop: isFemale ? 'bg-rose-500' : 'bg-blue-600',
    avatarBorder: isFemale ? 'border-rose-50' : 'border-blue-50',
    avatarIcon: isFemale ? 'text-rose-500' : 'text-blue-600',
    badgeBg: isFemale ? 'bg-rose-50' : 'bg-blue-50',
    badgeText: isFemale ? 'text-rose-600' : 'text-blue-600',
  };
  ...
  return (
    <div className="w-full flex flex-col items-center gap-8">
      ...
      <div className="w-full bg-white rounded-[2.5rem] shadow-sm border border-slate-200 overflow-hidden flex flex-col items-center text-center p-8 sm:p-12 relative mt-12 transition-all duration-500">
         
         {/* Fondo Superior Dinámico */}
         <div className={`absolute top-0 left-0 w-full h-32 ${theme.bgTop} transition-colors duration-500`}></div>
         
         {/* Avatar Circular Dinámico */}
         <div className={`z-10 h-28 w-28 bg-white rounded-full flex items-center justify-center mb-6 border-4 ${theme.avatarBorder} shadow-xl transition-colors duration-500`}>
           <User className={`h-12 w-12 ${theme.avatarIcon}`} />
         </div>
         ...
         <div className="mt-6 flex flex-col sm:flex-row justify-center items-center gap-3 w-full">
           <span className={`flex items-center justify-center gap-2 ${theme.badgeBg} ${theme.badgeText} px-6 py-3 rounded-2xl font-black text-xs sm:text-sm uppercase tracking-widest w-full sm:w-auto transition-colors duration-500`}>
