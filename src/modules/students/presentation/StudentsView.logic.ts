...
export const useStudentsLogic = () => {
    ...
    useEffect(() => {
        ...
        try {
            unsubAlumnos = StudentUseCases.obtenerAlumnosActivos(userData.campo, (data) => {
                ...
                setCargando(false);
                setAsistencia(prev => {
                    if (Object.keys(prev).length === 0 && data && data.length > 0) {
                        const inicial: Record<string, string> = {};
                        data.forEach(nino => { if(nino && nino.id) inicial[nino.id] = 'Presente'; });
                        return inicial;
                    }
                    return prev;
                });
        ...
        try {
            unsubAsistencias = StudentUseCases.suscribirAsistenciasActivas(userData.campo, (asistencias) => {
                const listaSegura = asistencias || [];
                
                const historialOrdenado = [...listaSegura].sort((a, b) => {
                ...
                const asistenciasPorCreacion = [...listaSegura].sort((a, b) => (b.createdAt || 0) - (a.createdAt || 0));
                ...
                if (asistenciasPorCreacion.length > 0) {
                    ...
                    if (ultima.fecha === fechaHoy) {
                        ...
                        setSeDioLeccion(ultima.leccionDada ?? true); 
    ...
    const obtenerRanking = () => { 
        ...
        (alumnos || []).forEach(a => { if(a && a.id) conteo[a.id] = 0; }); 
        validas.forEach(asis => { 
            if (asis && asis.registros) { 
    ...
    const obtenerHistorialPorMes = () => { 
        ...
        (historialAsistencias || []).forEach(asis => { 
            if (!asis || !asis.fecha || typeof asis.fecha !== 'string') return; 
    ...
    const obtenerAlumnosPorEdad = () => { return (alumnos || []).filter(a => { if(!a) return false; const edadStr = calcularEdadExacta(a.fechaNacimiento, a.edad as number); const edadNum = typeof edadStr === 'number' ? edadStr : parseInt(edadStr as string); if (isNaN(edadNum)) return false; if (edadMin !== '' && edadNum < edadMin) return false; if (edadMax !== '' && edadNum > edadMax) return false; return true; }); };
    const actualizarAsistencia = (id: string, estado: string) => { setAsistencia(prev => ({ ...prev, [id]: estado })); };
    const alumnosParaAsistencia = (alumnos || []).filter(alumno => { if (!alumno) return false; if (asistenciaDocId) return asistencia.hasOwnProperty(alumno.id!); return true; });
    ...
    const obtenerCumpleanerosPorMes = () => { 
        ...
        (alumnos || []).forEach(alumno => { 
            if (alumno && alumno.fechaNacimiento && typeof alumno.fechaNacimiento === 'string') { 
                const partes = alumno.fechaNacimiento.split('-');
                if (partes.length === 3) {
                    const mes = parseInt(partes[1], 10); 
                    if (!isNaN(mes) && agrupados[mes]) agrupados[mes].push(alumno); 
                }
            } 
        }); 
        ...
        Object.keys(agrupados).forEach(mes => { 
            agrupados[mes as any].sort((a, b) => { 
                const diaA = parseInt((a.fechaNacimiento || '').split('-')[2] || '0', 10); 
                const diaB = parseInt((b.fechaNacimiento || '').split('-')[2] || '0', 10); 
                return diaA - diaB; 
            }); 
        }); 
        return agrupados; 
    ...
    const abrirModalEditar = (alumno: Alumno) => { if (!alumno || !alumno.fechaNacimiento) return; const [year, month, day] = alumno.fechaNacimiento.split('-'); setForm({ nombre: alumno.nombre || '', birthDay: parseInt(day || '1', 10).toString(), birthMonth: parseInt(month || '1', 10).toString(), birthYear: year || currentYear.toString(), genero: alumno.genero || '' }); setEditandoId(alumno.id || null); setIsModalOpen(true); };
    ...
    return {
        alumnos: alumnos || [], alumnosParaAsistencia, cargando, form, setForm, isModalOpen, setIsModalOpen, abrirModalNuevo, abrirModalEditar, guardarAlumno, eliminarAlumno,
